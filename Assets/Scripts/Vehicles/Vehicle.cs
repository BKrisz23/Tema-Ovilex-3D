...
public class Vehicle : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        if(isAutoPilotActive) /// Dezactivarea autopilotului in timpul accelerației sau frânei
            if(isAccelerating || isBreaking) isAutoPilotActive = false;
    ...
    void wheelSmokeEffect(){ 
        ...
        if(isTyreSmokeActive && tyreSmoke.Length > 0 && isAccelerating && accelerationTimeElapsed > minAccelerationVFXTime){ /// Verfifică daca este o accelerație brusc
    ...
    public BeamLight ToggleBeamLights(){ /// Comută starea farurilor
        if(beamLightsCone.Length <= 0 || beamLightsStars.Length <= 1) return BeamLight.None;
        ...
        switch(beamLight){
            case BeamLight.Short: beamLightsStars[0].SetActive(true); break;
            case BeamLight.Long: beamLightsStars[1].SetActive(true);break;
            case BeamLight.None: beamLightsStars[0].SetActive(false); beamLightsStars[1].SetActive(false); break;
        }   
